...
var download_file_call_back = util.callbackify(downloadfile)
var compressing_function_call_back = util.callbackify(compressing_function)
var uploading_function_call_back = util.callbackify(uploading_function)
download_file_call_back("http://www.file.com/image.jpg", (err, ret) => {
    if (err)
        throw (err)
    else {
        compressing_function_call_back(ret, (err, ser) => {
            if (err)
                throw (err)
            else {
                uploading_function(ser, () => {})
            }
        })
    }
})